...
public class HtmlAnalyzer {
	...
	private void processIssueCommentBodyNodeList(NodeList list,
			String commentid) {
		// 迭代开始
				SimpleNodeIterator iterator = list.elements();
				while (iterator.hasMoreNodes()) {
	...
	private void processCommitIdNodeList(NodeList nodeList) {
		...
		while (iterator.hasMoreNodes()) {
			...
			// System.out.println(node.getText());
			if (node.getText().matches(
					"div class=\"commit-meta clearfix\".*+")) {// add
	...
	private String handleCommitCommentBody(Node node) {
		return node.getChildren().elementAt(3).getChildren()
				.elementAt(1).getChildren().elementAt(1).toPlainTextString().trim(); 
	}
