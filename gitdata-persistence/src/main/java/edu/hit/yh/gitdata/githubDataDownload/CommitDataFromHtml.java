...
public class CommitDataFromHtml {
	...
	public List<PushEvent> processPush(NodeList nodeList,List<PushEvent> pList){
		...
		while(sni.hasMoreNodes()){
			...
			if (node.getText().contains("class=\"commit full-commit")) {
				...
				//TODO 解析comment工作
				Node actorNode = DownloadUtil.getSomeChild(node, "rel=\"contributor\"");
				...
				Node titleNode = DownloadUtil.getSomeChild(node, "class=\"commit-desc\"");
				List<Node> artifactList = new ArrayList<Node>();
				artifactList = DownloadUtil.getLableList(node, "class=\"issue-link", artifactList);
				Pattern milestonePattern = Pattern.compile("href=\"https://github.com/[a-zA-Z]+/[a-zA-Z]+/[a-zA-Z]+/[0-9]+");
				String title = "";
	 ...
	 * @param source
	 */
	private List<CommitCommentEvent> processComment(NodeList nodeList,List<CommitCommentEvent> ccList){
		SimpleNodeIterator sni = nodeList.elements();
		while(sni.hasMoreNodes()){
			Node node = sni.nextNode();
	...
	public static void main(String args[]){
		...
		String url = "https://github.com/jquery/jquery/commit/56bb677725b21415905e5c3eeb1e05be4480e780";
		NodeList nodeList = htmlAnalyzer.getNodeList(url);
